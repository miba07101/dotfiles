...
require("lazy").setup({
  ...
  spec = { -- {{{
    ...
    { "zk-org/zk-nvim", -- {{{
      ...
      keys = { -- {{{
        { "<leader>nn", mode = "n", function() vim.ui.input({ prompt = "Title: " }, function(title) if not title then return end if title ~= "" then require("zk").new({ title = title, dir = osvar.ObsidianPath() .. "/00-inbox" }) else return end end) end, desc = "ZK New Note", noremap = true, silent = true},
