...
function _G.DetectOsType() -- {{{
	...
	local function PythonInterpreter()
		return venv and (os_type == "windows" and (venv .. "\\Scripts\\python.exe") or (venv .. "/bin/python"))
			or "python3"
	end
...
map({ "n", "t" }, "<leader>ti", function()
	toggle_terminal("ipython --no-autoindent")
...
vim.api.nvim_create_user_command("ConvertMarkdownToPython", function() -- {{{
	...
	vim.defer_fn(function()
      ...
      (import_from_statement) @imp
		...
		table.sort(import_nodes, function(a, b)
				...
				dashboard = { -- {{{
					...
					preset = {
						keys = {
							...
							-- { icon = " ", key = "n", desc = "New File", action = ":lua vim.ui.input({ prompt = "Title: " }, function(i) if i then vim.cmd("enew | file " .. i .. " | startinsert") end end)" },
							{
								icon = " ",
								key = "n",
								desc = "New File",
								action = ':lua vim.ui.input({ prompt = "Title: " }, function(i) if i then vim.cmd("enew | file " .. i .. " | startinsert") end end)',
							},
							...
							},
							{
								...
								key = "p",
								desc = "Projects",
								action = ":lua Snacks.dashboard.pick('projects', {dev = '~/git-repos/', confirm='picker', recent = false})",
							},
		...
		-- {{{ [ Mini.nvim collection ]
		{
			...
			config = function()
				...
				require("mini.comment").setup({ -- {{{
					options = {},
					mappings = (osvar.os_type == "linux") and {
						comment = "<C-/>",
						comment_line = "<C-/>",
						comment_visual = "<C-/>",
						textobject = "<C-/>",
...

		{
			...
			keys = { -- {{{
				...
				},
				{
					...
					desc = "open in browser (html only)",
					noremap = true,
					silent = true,
				},
...
vim.api.nvim_create_user_command("LspInfo", function()
	...
	for _, client in ipairs(clients) do
		...
		local cmd_display = type(client.config.cmd) == "table" and table.concat(client.config.cmd, " ") or "<function>"
